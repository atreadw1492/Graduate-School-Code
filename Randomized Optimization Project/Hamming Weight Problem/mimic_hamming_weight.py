...
x = Mimic( domain = [(0,1)]*50 , fitness_function=get_fitness, samples = 3000, percentile = 0.8)
...
by_percentile = [Mimic( domain = [(0,1)]*50 , fitness_function=get_fitness, samples = 200, percentile = elt) for elt in percentiles]
...
# 90% percentile showed the worst performance of those tested, so we will test if adjusting the number
# of iterations changes the its result performance
# Test varying the number of iterations
num_iterations_list = [15,20,25]
mimic_obj = Mimic( domain = [(0,1)]*50 , fitness_function=get_fitness, samples = 200, percentile = 0.9)
num_iterations_results = [mimic_obj.fit(n = num) for num in num_iterations_list]


num_iterations_max_values = [max(map(get_fitness , result)) for result in num_iterations_results]

start = time.time()
x = Mimic( domain = [(0,1)]*50 , fitness_function=get_fitness, samples = 200, percentile = 0.3)
x_fit = x.fit(n = 10)
end = time.time() - start
#
